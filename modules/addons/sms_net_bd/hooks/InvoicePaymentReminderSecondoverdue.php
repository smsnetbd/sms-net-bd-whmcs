    ...
    function InvoicePaymentReminderSecondoverdue($args)
    {
        if ($args['type'] == "secondoverdue") {
            ...
            if (empty($settings['api_key'])) {
                logActivity('sms.net.bd - InvoicePaymentReminderSecondoverdue :  ' . 'No API Key Provided', 0);
                return null;
            }
        } else {
            return false;
        ...
        if ($num_rows == 1) {
            ...
            if (!$class->validatePhoneNumber($UserInformation['gsmnumber'])) {
                logActivity('sms.net.bd - InvoicePaymentReminderSecondoverdue :  ' . 'Invalid phone number Provided', 0);
                return null;
            }
