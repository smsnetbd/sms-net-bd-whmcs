...
if (!function_exists('AfterRegistrarRegistrationAdmin')) {
    function AfterRegistrarRegistrationAdmin($args)
    {
        ...
        if (empty($settings['api_key'])) {
            logActivity('sms.net.bd - AfterRegistrarRegistrationAdmin :  ' . 'No API Key Provided', 0);
            return null;
        }
        ...
        foreach ($admin_numbers as $gsm) {
            if (!empty($gsm)) {
                ...
                if (!$class->validatePhoneNumber($gsm)) {
                    logActivity('sms.net.bd - AfterRegistrarRegistrationAdmin :  ' . 'Invalid phone number Provided', 0);
                    continue;
                }
