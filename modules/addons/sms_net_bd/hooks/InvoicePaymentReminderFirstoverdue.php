    ...
    function InvoicePaymentReminderFirstoverdue($args)
    {
        if ($args['type'] == "firstoverdue") {
            ...
            if (empty($settings['api_key'])) {
                logActivity('sms.net.bd - InvoicePaymentReminderFirstoverdue :  ' . 'No API Key Provided', 0);
                return null;
            }
        } else {
            return false;
        ...
        if ($num_rows == 1) {
            ...
            if (!$class->validatePhoneNumber($UserInformation['gsmnumber'])) {
                logActivity('sms.net.bd - InvoicePaymentReminderFirstoverdue :  ' . 'Invalid phone number Provided', 0);
                return null;
            }
